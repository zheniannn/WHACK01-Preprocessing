...
def _gap_sensitivity_report(day_result: dict, min_duration_s: float, min_points: int) -> None:
    """Rerun segmentation in-memory on one already-cleaned day at gap-split
    60/120/180s and report num_segments/points_kept/median_duration for each.
    Informational only -- does not change the configured default.
    """
    from utils.trajectory_prep import assign_segments, filter_valid_segments, remove_glitch_points

    # Re-segments the day's final (already cleaned) points at each gap value.
    ...
    for gap in (60, 120, 180):
        ...
        final = filter_valid_segments(cleaned, min_duration_s, min_points)
        if final.empty:
            print(f"{gap:>12} | {0:>12} | {0:>18} | {0.0:>18}")
            continue
        n_segments = final["segment_id"].nunique()
        n_points = len(final)
        median_dur = float(final.groupby("segment_id")["segment_duration_s"].first().median())
        print(f"{gap:>12} | {n_segments:>12} | {n_points:>18} | {median_dur:>18.1f}")
...
def main() -> None:
    ...
    for date, input_path in day_files:
        ...
        print(f"rows after freshness filter:{result['rows_after_freshness']:>10}")
    ...
    # Sanity assertion: the onground filter should have removed a nonzero
    # number of rows on at least one day, else parse_onground is likely broken.
    onground_removed_by_day = {r["date"]: r["_onground_removed"] for r in day_results}
    assert max(onground_removed_by_day.values()) > 0, (
        f"onground filter removed 0 rows on every day {onground_removed_by_day} -- "
        f"parse_onground() is likely misinterpreting the column."
    )
    for date, n in onground_removed_by_day.items():
        print(f"\nonground rows removed ({date}): {n}")
    ...
    print("=" * 70)
    _gap_sensitivity_report(day_results[0], args.min_duration, args.min_points)
